...
<div class="col-md-12">
<br>
<h3 align="center">Marriage Certificate Data</h3><br>
...
@endif
    ...
    <th scope="col">B_Nationality</th>
      ...
      <th scope="col">Marriage_Date</th>
      <th scope="col">Solemnized_Name</th>
      <th scope="col">Solemnized_CNIC</th>
...
<td>{{$row['solemnized_name']}}</td>
<td>{{$row['solemnized_cnic']}}</td>
<td>{{$row['apply_date']}}</td>



<td>
<form method="post" action="{{action('marriagecontroller@destroy', $row['id'])}}">
@csrf
...
<a href="{{ action ('marriagecontroller@edit', $row ['id'])}}"><button type="submit" class="btn btn-success" >Report</button></a>
</td>
