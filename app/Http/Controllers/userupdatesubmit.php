...
use Appp\admin;
class userupdatesubmit extends Controller
{
    ...
    $this->validate($req,['contact'=>'required|digits:11'],[
    
      'contact.required'=> 'Contact number cant be empty',
      'contact.digits' => 'Must Be digit 11 are you mad',
    ...
    $this->validate($req,['image'=>'required'],[
     ...
     DB::update('update useres set name=?, lname=?, email=?, password=?, conpassword=?, contact=?, gender=?, city=?, country=?, birth=? ,image=? where id=?'
     ,[$name,$lname,$email,$password,$conpassword,$contact, $gender, $city,$country,$birth,$image,$id]);


     $data=DB::select('select * from useres where email=? and password=?',[$email,$password]);
     if(count($data))
     {       
       ...
       return redirect('lawyerpage.userlogin');
